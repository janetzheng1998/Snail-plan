...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return json({ error: "还没有配置 DeepSeek API Key。" }, 503);
  }

  const body = (await request.json().catch(() => null)) as OrganizeRequestBody | null;
  ...
  if (!text) {
    return json({ error: "请输入本次记录内容。" }, 400);
  }
  ...
  if (!organized) {
    return json({ error: "AI 返回格式异常，请重试。" }, 502);
  }
